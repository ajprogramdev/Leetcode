...
A peak is an element that is greater than both of its neighbors.
 If it is the first or last element, it is a peak if it is greater than its one neighbor. Assume the array has a length of at least 2.
...
Approach:
1.Edge Cases
2.Create an array to store the peaks.
3.Loop through the array 
    compare the index value with its left and right index values.
    if greater than left and right, push to array 
        if no value on one side, indexValue is larger than the other side, push to array.

4. return array peaks
*/
